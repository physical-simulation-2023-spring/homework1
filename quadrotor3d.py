...
FillData(rel_loc, [[.2, 0, .2], [.2, 0, -.2], [-.2, 0, -.2], [-.2, 0, .2]])

# The global transformation of the object.
transform = Transform3D()
# You may or may not want to use the following variable.
rotvec = ti.Vector.field(3, float, shape=())
...
@ti.func
def ForwardEuler():
    q = {"translation": transform.translation[None], "rotmat": transform.rotation[None], "rotvec": rotvec[None], 
...
def YControllor(y_goal: float) -> float:
    ...
    return (- 60 * (y - target) - 10 * dot_y + 0.25) * gravity
